...
def popola():
    ...
    prodotti = [
        ...
        # MATERIALI A PESO (qli)
        ("Cubetti 6/8 misto", "qli", 11.50),
        ("Cubetti 8/10 misto", "qli", 11.00),
        ("Liste 6/8 misto", "qli", 11.00),
        ("Liste 8/10 misto", "qli", 11.00),
        ("Binderi 6/8 misto", "qli", 16.00),
        ("Binderi 8/10 misto", "qli", 16.00),
    ...
    for nome, um, prezzo in prodotti:
        # Controlla manualmente se il prodotto esiste già prima di inserirlo
        cursor.execute("SELECT 1 FROM prodotti WHERE nome = %s", (nome,))
        exists = cursor.fetchone()
        
        if not exists:
            cursor.execute("""
                INSERT INTO prodotti (nome, unita_misura, prezzo_base) 
                VALUES (%s, %s, %s);
            """, (nome, um, prezzo))
